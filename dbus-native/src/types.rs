...
use std::fmt;
use std::io::{ErrorKind, IoSlice, Result as IoResult, Seek, SeekFrom, Write};

use dbus_strings::{
    DBusStr, InvalidStringError, ObjectPath, SignatureMulti as Signature, SignatureSingle,
    StringLike,
};
...
pub fn align_buf<M: Marshal>(a: &[u8]) -> Result<&[u8], &'static str> {
    ...
    let z = n - p;
    if z >= a.len() {
        Err("Not enough message data (while aligning)")?
    }
    Ok(&a[(n - p)..])
}
...
impl From<InvalidStringError> for DemarshalError {
    fn from(_: InvalidStringError) -> DemarshalError {
        DemarshalError::InvalidString
    }
}
...
impl<B: Write + Seek> MarshalState<B> {
    ...
    pub fn write_fixed(&mut self, align: usize, data: &[u8]) -> IoResult<()> {
        self.write_vectored(&[IoSlice::new(self.align_buf(align)), IoSlice::new(data)])
    }
    pub fn write_array<F: FnOnce(&mut Self) -> IoResult<()>>(
        &mut self,
        el_align: usize,
        f: F,
    ) -> IoResult<()> {
        let len_pos = self.pos;
        self.write_vectored(&[IoSlice::new(self.align_buf(4)), IoSlice::new(&[0, 0, 0, 0])])?;
...
impl<'a> DemarshalState<'a> {
    ...
    pub fn read_array(&mut self, el_align: usize) -> Result<DemarshalState<'a>, DemarshalError> {
        if self.signature.as_bytes().get(0) != Some(&b'a') {
            Err(DemarshalError::WrongType)?
        };
        let x = self.read_single(4, 4)?;
        let x: [u8; 4] = x.try_into().unwrap();
        let arr_size = (if self.is_big_endian {
            u32::from_be_bytes(x)
        } else {
            u32::from_le_bytes(x)
        }) as usize;
    ...
    pub fn read_variant(&mut self) -> Result<DemarshalState<'a>, DemarshalError> {
        ...
        let r = std::str::from_utf8(r).map_err(|_| DemarshalError::InvalidString)?;
        self.pos = new_pos;
        SignatureSingle::new(r).map_err(|_| DemarshalError::InvalidString)?;
...
impl<'a> Demarshal<'a> for &'a DBusStr {
    fn read_buf(b: &mut DemarshalState<'a>) -> Result<Self, DemarshalError> {
        ...
        Ok(DBusStr::new(r).map_err(|_| DemarshalError::InvalidString)?)
    }
}
